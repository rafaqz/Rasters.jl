"""
    zonal(f, x::RasterStackOrArray, obj)

Calculate zonal statistics for an object or table.

# Arguments

- `f`: aby function that reduces an iterable to a single value, such as `Base.sum` or `Statistic.mean`
- `x`: A `Raster` or `RasterStack`
- `obj`: A `Raster`, `RasterStack`, dim tuple, extent or GeoInterface.jl compatible geometry.
...
st = read(RasterStack(WorldClim{Climate}; month=Jan))
