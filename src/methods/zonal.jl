...
```
"""
function zonal(f, x::RasterStack; of, skipmissing=true, spatialslices=_True(), missingval = missingval(x), kw...)
...
function zonal(f, x::Raster; of, skipmissing=true, spatialslices=_True(), missingval = missingval(x), kw...)
    open(x) do xo
