...
@testset "step" begin
    ...
    @test Rasters._cdm_span(f32_indices, ForwardOrdered())[1] === Regular(0.05)

    f32_indices_rev = range(10.075f0, 0.075f0; step = -0.05f0) |> collect
    @test Rasters._cdm_span(f32_indices_rev, ReverseOrdered())[1] === Regular(-0.05)

    # test if regular indices are not rounded when they should not
    indices_one_third = range(0, 10; length = 31) |> collect
    @test Rasters._cdm_span(indices_one_third, ForwardOrdered())[1] === Regular(1/3)
