...
@testset "warp" begin
    ...
    @testset "default missing" begin
        ...
        missingval(warped) === missing
        @test warped[1, 1] === warped[1, end] === warped[end, 1] === warped[end, end] === missing === missingval(warped)
    end

    @testset "custom missing" begin
        warped = warp(r, Dict(:t_srs => "EPSG:25832"); missingval=0xff => 0xff)
        @test warped isa Raster
        @test size(warped) == (720, 721)
        # the crs is rotated so the image is rotated an all four corners should be missing
